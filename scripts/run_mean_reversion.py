#!/usr/bin/env python3
"""Run a transparent mean-reversion backtest on liquid ETFs."""
...
from quant_research.signals import mean_reversion_signal
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--start", default="2010-01-01", help="Inclusive start date for historical data.")
    parser.add_argument("--end", default=None, help="Exclusive end date for historical data.")
    parser.add_argument(
        "--lookback",
        type=int,
        default=20,
        help="Rolling z-score lookback window in trading days.",
    )
    parser.add_argument(
        "--entry-threshold",
        type=float,
        default=1.5,
        help="Enter long below -threshold and short above +threshold.",
    )
    parser.add_argument(
        "--exit-threshold",
        type=float,
        default=0.5,
        help="Exit as the z-score normalizes back inside +/- exit threshold.",
    )
    parser.add_argument(
        "--signal-input",
        choices=["price", "return"],
        default="price",
        help="Whether to compute the rolling z-score on prices or simple returns.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--equity-chart-name",
        default="mean_reversion_equity_curve.png",
        help="Filename for the equity-curve chart.",
    )
    parser.add_argument(
        "--drawdown-chart-name",
        default="mean_reversion_drawdown.png",
...
def save_charts(
    *,
    ...
    ax.plot(strategy_equity.index, strategy_equity, label="Mean Reversion Strategy")
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    ax.plot(strategy_drawdown.index, strategy_drawdown, label="Mean Reversion Strategy")
...
def main() -> None:
    """Run the end-to-end mean-reversion research workflow."""
